...
#
# Xray-script:
#   https://github.com/hors803/Xray-script
# =============================================================================
...
function apply_anti_detection_profile() {
    local target_domain="$1"
    local browser_host="${target_domain%%:*}"
    local browser_accept_language="en-US,en;q=0.9"

    [[ -n "${browser_host}" && "${browser_host}" != "null" ]] || return 0

    XRAY_CONFIG="$(echo "${XRAY_CONFIG}" | jq --arg host "${browser_host}" --arg accept_language "${browser_accept_language}" '
        def ensure_rule($rule):
            .routing.rules = ((.routing.rules // []) |
                if any(.ruleTag? == $rule.ruleTag) then
                    map(if .ruleTag? == $rule.ruleTag then . + $rule else . end)
                else
                    [$rule] + .
                end);
        ...
        | .outbounds |= map(
            if .tag == "direct" then
                .settings = ((.settings // {}) + {"domainStrategy": "UseIPv4"})
            else
                .
            end
        )
        ...
        | reduce range(0; (.inbounds | length)) as $i (.;
            if .inbounds[$i].streamSettings.network? == "xhttp" then
                ...
                | .inbounds[$i].streamSettings.xhttpSettings.host = $host
                | .inbounds[$i].streamSettings.xhttpSettings.headers = {
                    "accept": "*/*",
                    ":method": "GET",
                    ":scheme": "https",
                    ":authority": $host,
...
function handler_xray_config() {
    ...
    case "${CONFIG_TAG,,}" in
    xhttp | trojan | fallback | sni)
        apply_anti_detection_profile "${TARGET_DOMAIN}"
        ;;
    esac
